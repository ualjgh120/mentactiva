...
**Autenticación por PIN de 4 dígitos:** Pantalla `/acceso` carga todos los perfiles de Supabase (tabla `users`) → los muestra en grid con avatar emoji y nombre → al pulsar un perfil, guarda el `userId` en `sessionStorage` (`menteactiva_selected_user`) → navega a `/pin` → teclado numérico virtual con grid 3×4 (1-9, vacío, 0, borrar) + listener `window.keydown` para teclado físico → al completar 4 dígitos, ejecuta `validateUserPin()` que realiza una **verificación segura mediante comparación de hashes con `bcrypt.compareSync()`** → si es válido: `setAuthenticatedUser(userId)` en `sessionStorage` (`menteactiva_authenticated_user`) + `clearSelectedUser()` + `navigate('/perfil')` → si es inválido: muestra error y resetea el PIN. El `ProtectedRoute` verifica la sesión con `getAuthenticatedUser()` (busca el `userId` en `sessionStorage` → `getUserById()` en Supabase) en cada renderizado.

**Creación de perfiles:** Formulario único con 3 campos: nombre (mínimo 2 caracteres), PIN (validación regex `^\d{4}$`, input con `inputMode="numeric"` y `maxLength={4}`, filtra no-dígitos con `replace(/\D/g, '')`) y avatar (selector visual de 6 opciones emoji con indicador de selección). Al hacer submit, la función `createUser(name, pin, avatar)` **genera un salt y hashea el PIN con `bcrypt` (coste 10)** antes de realizar el `INSERT` en la tabla `users` de Supabase → tras el éxito, hace `setSelectedUser(newUser.id)` y navega a `/pin` para el primer acceso.
